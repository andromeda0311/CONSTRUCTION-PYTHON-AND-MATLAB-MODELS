...
try
    opts = detectImportOptions('MEPFS_Total_Cost.csv');
    opts.VariableNamingRule = 'preserve';
    T = readtable('MEPFS_Total_Cost.csv', opts);
...
h.Title = 'Correlation Matrix of Raw Numeric Features (MEPFS)';
h.XLabel = 'Features'; h.YLabel = 'Features'; h.CellLabelFormat = '%.2f'; h.FontSize = 8;
saveas(gcf, 'visualizations/correlation_matrix_before_mepfs.png');
disp("Saved: correlation_matrix_before_mepfs.png");
...
title('Distribution of Budget (Skewed)'); xlabel('Budget'); ylabel('Frequency'); grid on;
subplot(1, 2, 2);
histogram(T_clean.budget_log, 50, 'FaceColor', '#D95319');
title('Distribution of Log-Transformed Budget (Normalized)'); xlabel('Log(Budget + 1)'); ylabel('Frequency'); grid on;
saveas(gcf, 'visualizations/budget_distributions_comparison_mepfs.png');
disp("Saved: budget_distributions_comparison_mepfs.png");
...
for i = 1:length(vars_to_fill)
    ...
    % --- UPGRADE: Use mean for imputation of classrooms/storeys as it might be more representative ---
    impute_val = round(mean(T_clean.(varName), 'omitnan'));
    T_clean.(varName) = fillmissing(T_clean.(varName), 'constant', impute_val);
end

% --- START OF UPGRADE: Advanced Feature Engineering for MEPFS ---
...
% Create powerful "per-unit" features based on insights
...
% Handle potential division by zero if num_classrooms is 0
...
% --- START OF UPGRADE: Revise feature list to reduce multicollinearity ---
% We now use the more powerful, engineered features and remove the redundant individual components.
...
title('Correlation Matrix of Final Engineered Features (MEPFS - Upgraded)');
saveas(gcf, 'visualizations/correlation_matrix_after_mepfs_upgraded.png');
disp("Saved: correlation_matrix_after_mepfs_upgraded.png");
...
% --- UPGRADE: Make the baseline model slightly more powerful ---
baseline_model = fitrensemble(X_train_poly_scaled, y_train, 'Method', 'LSBoost', 'NumLearningCycles', 250, 'LearnRate', 0.1);
% --- END OF UPGRADE ---
...
title('Top 20 Feature Importances (Baseline MEPFS Model - Upgraded)');
xlabel('Predictor Importance');
set(gca, 'YDir','reverse');
saveas(gcf, 'visualizations/feature_importance_baseline_mepfs_upgraded.png');
disp("Saved: feature_importance_baseline_mepfs_upgraded.png");
...
% --- All visualization and saving code remains the same from here ---
% --- Filenames are changed to reflect the "mepfs" analysis ---
figure('Visible', 'off');
scatter(y_test_actual, final_predictions, 'filled');
hold on; plot([min(y_test_actual), max(y_test_actual)], [min(y_test_actual), max(y_test_actual)], 'r--', 'LineWidth', 2);
title('Actual vs. Predicted Budget (MEPFS)'); xlabel('Actual Budget'); ylabel('Predicted Budget');
legend('Predictions', 'Ideal Fit', 'Location', 'northwest'); grid on;
saveas(gcf, 'visualizations/actual_vs_predicted_mepfs.png');
disp("Saved: actual_vs_predicted_mepfs.png");

residuals = y_test_actual - final_predictions;
figure('Visible', 'off');
scatter(final_predictions, residuals, 'filled');
hold on; yline(0, 'r--', 'LineWidth', 2);
title('Residual Plot (MEPFS)'); xlabel('Predicted Budget'); ylabel('Residuals (Actual - Predicted)'); grid on;
saveas(gcf, 'visualizations/residual_plot_mepfs.png');
disp("Saved: residual_plot_mepfs.png");
...
yticks(1:top_n); yticklabels(selected_feature_names(sorted_idx_final(1:top_n)));
title('Top 20 Feature Importances (Final Optimized MEPFS Model)'); xlabel('Predictor Importance'); set(gca, 'YDir','reverse');
saveas(gcf, 'visualizations/feature_importance_final_mepfs.png');
disp("Saved: feature_importance_final_mepfs.png");

figure('Visible', 'off');
metrics_data = [r2_base, r2_best; mae_base, mae_best];
b = bar(metrics_data);
xticklabels({'R-squared', 'MAE'}); legend({'Baseline Model', 'Optimized Model'});
title('Model Performance Comparison (MEPFS)'); b(2).FaceColor = '#D95319'; grid on;
saveas(gcf, 'visualizations/performance_comparison_mepfs.png');
disp("Saved: performance_comparison_mepfs.png");
...
save('mepfs_model_assets.mat', ...
    'optimized_model', 'mu', 'sigma', 'selected_mask', ...
    'final_feature_columns', 'poly_feature_names');
disp("All model assets saved to 'mepfs_model_assets.mat'.");
