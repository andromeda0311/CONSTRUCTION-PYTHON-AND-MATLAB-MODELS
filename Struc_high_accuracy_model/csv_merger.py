...
def process_cost_files(quantity_file, unit_cost_file, output_file):
    ...
    start_col_index_qty = quantity_df.columns.get_loc('Structural aspect') + 1
    numeric_quantity = quantity_df.iloc[:, start_col_index_qty:-2].copy()

    start_col_index_unit = unit_cost_df.columns.get_loc('Structural aspect') + 1
...
# The script expects the CSV files to be in the same directory where it is run.
quantity_filename = r'Structural Cost\high_accuracy_model\Stuctural Quantity Cost.csv'
unit_cost_filename = r'Structural Cost\high_accuracy_model\Stuctural Quantity Cost.csv'
output_filename = 'Structural_Total_Cost.csv'
